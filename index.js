  ...
  async runAction() {
    return new Promise(async (resolve, reject) => {
      ...
      const app_to_recycle = apps.find(
        (app) =>
          app.github === issue.pull_request.html_url &&
          app.redirect_uri !== preview_url
      );
      ...
      const getRecyclableApp = () => {
        const open_pull_requests_urls = open_pull_requests.map(
          (pr) => pr.html_url
        );
        return apps.find(
          (app) => app.github && !open_pull_requests_urls.includes(app.github)
        );
      };

      const existing_app = app_to_recycle || getRecyclableApp();
      const stripped_app_title = issue.title
        .replace(/[^A-Za-z0-9 ]/g, "")
        .substring(0, 35);
      const app_options = {
